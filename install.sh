...
install_lua_packages() {
	...
	for package in "${lua_packages[@]}"; do
		print_info "Installing $package..."
		# Install to mise lua tree (default luarocks behavior with mise-installed lua)
		if luarocks install "$package" 2>/dev/null; then
			print_success "$package installed"
...
install_cargo_tools() {
	...
	print_info "Tools include: git-delta (better git diff), eza (better ls)"
...
verify_installation() {
	...
	# Check formatters (from conform.nvim config)
	echo ""
	color_echo "${BOLD}🎨 Code Formatters:${NC}"

	local formatters=(
		"prettier:Prettier (JS/TS/CSS/HTML/YAML/Markdown)"
		"eslint_d:ESLint daemon (JS/TS)"
		"black:Black (Python)"
		"isort:isort (Python imports)"
		"shfmt:shfmt (Shell)"
		"rubocop:RuboCop (Ruby)"
		"standardrb:StandardRB (Ruby)"
		"erb-formatter:ERB Formatter (ERB templates)"
	)

	for formatter_spec in "${formatters[@]}"; do
		IFS=':' read -r cmd name <<<"$formatter_spec"
		if command_exists "$cmd"; then
			printf "  ${GREEN}✓${NC} %-35s installed\n" "$name"
		else
			printf "  ${YELLOW}○${NC} %-35s ${YELLOW}not installed (optional)${NC}\n" "$name"
			((warnings++))
		fi
	done
