...
describe("modules.lsp #integration", function()
		...
		package.preload["mason-lspconfig"] = function()
			return {
				setup = function(config)
					...
					-- If handlers are provided, call them for test servers
					if config.handlers and config.handlers[1] then
						local test_servers = { "lua_ls", "ts_ls", "pyright" }
						for _, server in ipairs(test_servers) do
							config.handlers[1](server)
						end
					end
				end,
				setup_handlers = function(handlers)
					-- Legacy API - simulate calling default handler for some servers
					if handlers[1] then
						handlers[1]("lua_ls")
						handlers[1]("ts_ls")
						handlers[1]("pyright")
	...
	describe("Full setup workflow", function()
		...
		it("should configure all components in correct order", function()
			...
			package.preload["mason-lspconfig"] = function()
				return {
					setup = function(config)
						...
						-- Execute handlers for test servers
						if config.handlers and config.handlers[1] then
							config.handlers[1]("lua_ls")
						end
					end,
					get_installed_servers = function()
						return { "lua_ls" }
			...
			-- Order should be: mason -> diagnostics -> mason-lspconfig -> vim_lsp_config_lua_ls -> vim_lsp_enable_lua_ls
			-- With the handler-based approach, servers are configured immediately during mason-lspconfig.setup
			assert.equal("mason", setup_order[1])
			assert.equal("diagnostics", setup_order[2])
			assert.equal("mason-lspconfig", setup_order[3])
			assert.equal("vim_lsp_config_lua_ls", setup_order[4])
			assert.equal("vim_lsp_enable_lua_ls", setup_order[5])
		end)
	end)
	...
	describe("Configuration options", function()
		...
		it("should disable automatic_installation by default", function()
			...
			-- We now control server enablement via handlers, not automatic_installation
