		...
		file_history_panel = {
			{
				"n",
				"g!",
				'<cmd>lua require("diffview").open_commit_log()<cr>',
				{ desc = "Open commit log" },
			},
			{
				"n",
				"<C-A-d>",
				'<cmd>lua require("diffview").show_diff_for_range()<cr>',
				{ desc = "Show range diff" },
			},
			{
				"v",
				"<C-A-d>",
			...
			},
			{
				"n",
				"y",
				'<cmd>lua require("diffview").copy_hash()<cr>',
		...
		option_panel = {
			{
				"n",
				"<tab>",
				'<cmd>lua require("diffview.config").actions.select_entry()<cr>',
				{ desc = "Select option" },
			},
			{
				"n",
				"q",
				'<cmd>lua require("diffview.config").actions.close()<cr>',
				{ desc = "Close panel" },
			},
		},
	},
...
function M.setup(config)
	...
	if not setup_ok then
		vim.notify(string.format("Failed to setup diffview: %s", err), vim.log.levels.ERROR)
		return false
	end
