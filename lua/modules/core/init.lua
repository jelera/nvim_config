...
M.custom_keymaps = require("modules.core.custom_keymaps")
M.autocmds = require("modules.core.autocmds")
M.commands = require("modules.core.commands")
M.quality_of_life = require("modules.core.quality_of_life")
M.abbreviations = require("modules.core.abbreviations")
...
function M.setup(config)
	...
	-- Setup custom keymaps (workflow-specific key bindings from old vimrc)
	local custom_keymaps_success = M.custom_keymaps.setup()
	if not custom_keymaps_success then
		vim.notify("Core module setup failed: custom_keymaps", vim.log.levels.ERROR)
		return false
	end
	...
	-- Setup quality of life features (auto-save, last position, etc.)
	local qol_success = M.quality_of_life.setup()
	if not qol_success then
		vim.notify("Core module setup failed: quality_of_life", vim.log.levels.ERROR)
		return false
	end

	-- Setup abbreviations (typo corrections, dates, signatures)
	local abbrev_success = M.abbreviations.setup()
	if not abbrev_success then
		vim.notify("Core module setup failed: abbreviations", vim.log.levels.ERROR)
		return false
	end
